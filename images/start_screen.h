/*
 * Exported with nin10kit v1.7
 * Invocation command was nin10kit --mode=3 --resize=240x160 images/start_screen images/start_screen.png 
 * Time-stamp: Saturday 11/16/2019, 18:50:07
 * 
 * Image Information
 * -----------------
 * images/start_screen.png 240@160
 * 
 * All bug reports / feature requests are to be filed here https://github.com/TricksterGuy/nin10kit/issues
 */

#ifndef START_SCREEN_H
#define START_SCREEN_H

extern const unsigned short start_screen[38400];
#define START_SCREEN_SIZE 76800
#define START_SCREEN_LENGTH 38400
#define START_SCREEN_WIDTH 240
#define START_SCREEN_HEIGHT 160

#endif

